...
import { Avatar, Box, Typography, Stack, Button, IconButton, Menu, MenuItem, ListItemIcon, ListItemText } from "@mui/material";
...
export const CommentCard = ({ comment, depth = 0 }: CommentCardProps) => {
    ...
    const { softDelete, mutateLike, isLiked, likeCount, isLikePending } = useComments(comment.article_id, comment.id); // 削除用関数を取得
        ...
        >
            ...
            >
                ...
                <Typography variant="body2" sx={{ whiteSpace: "pre-wrap", color: "text.primary", mb: 1, pr: 1 }}>
                    {comment.is_deleted ? (
                        <span style={{ fontStyle: "italic", color: "gray" }}>このコメントは削除されました</span>
                    ) : (
                        comment.body
                    )}
                </Typography>
