...
export const AuthSettings = () => {
    ...
    const handleOauthTabChange = (event: React.SyntheticEvent, newValue: number) => {
        setOauthTab(newValue);
    };

    return (
        <>
            <Typography variant="h5" sx={{ mb: 3 }}>認証機構設定</Typography>
            <Divider sx={{ mb: 2 }} />
            ...
            <Box sx={{ mt: 2 }}>
                ...
                {mainTab === 4 && (
                    <>
                        <Tabs value={oauthTab} onChange={handleOauthTabChange} sx={{ mb: 1 }}>
                            <Tab label="Discord" />
                            <Tab label="Twitter" />
                            <Tab label="GitHub" />
                            <Tab label="Custom" />
                        ...
                        <Box sx={{ mt: 2 }}>
                            ...
                            {oauthTab === 3 && <Typography>その他OAuth2 用設定フォーム</Typography>}
                        </Box>
                    </>
                )}
            </Box>
