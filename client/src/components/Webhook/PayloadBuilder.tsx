...
export const templateToNode = (value: unknown): PayloadNode => {
  ...
  if (Array.isArray(value))
    return { type: "array", items: value.map(templateToNode) };
  if (typeof value === "object")
    return {
      type: "object",
      properties: Object.entries(value).map(([key, child]) => ({
        key,
        value: templateToNode(child),
      })),
    };
  ...
  switch (node.type) {
    case "object":
      return Object.fromEntries(
        node.properties.map((p) => [p.key, nodeToTemplate(p.value)]),
      );
    case "array":
      return node.items.map(nodeToTemplate);
    case "variable":
...
const NodeEditor = ({
  ...
  return (
    <Paper variant="outlined" sx={{ p: 1.5, width: "100%" }}>
      ...
      >
        {label && (
          <Typography sx={{ minWidth: 54 }} color="text.secondary">
            {label}
          </Typography>
        )}
        <Select
          size="small"
          value={node.type}
          onChange={(e) => setType(e.target.value as PayloadNode["type"])}
          sx={{ minWidth: 130 }}
        >
          <MenuItem value="variable">
            {t("webhooks.nodeTypes.variable")}
          </MenuItem>
          <MenuItem value="text">{t("webhooks.nodeTypes.text")}</MenuItem>
          <MenuItem value="number">{t("webhooks.nodeTypes.number")}</MenuItem>
          <MenuItem value="boolean">{t("webhooks.nodeTypes.boolean")}</MenuItem>
          <MenuItem value="null">{t("webhooks.nodeTypes.null")}</MenuItem>
          <MenuItem value="object">{t("webhooks.nodeTypes.object")}</MenuItem>
          <MenuItem value="array">{t("webhooks.nodeTypes.array")}</MenuItem>
        </Select>
        {node.type === "variable" && (
          <Select
            size="small"
        ...
        >
          {node.properties.map((property, index) => (
            ...
            >
              <TextField
                size="small"
                label={t("webhooks.property")}
                value={property.key}
                onChange={(e) => {
                  const properties = [...node.properties];
                  properties[index] = { ...property, key: e.target.value };
          ...
          <Button
            ...
            onClick={() =>
              onChange({
                ...
                properties: [
                  ...node.properties,
                  { key: "property", value: newNode("variable") },
                ],
              })
            }
      ...
      {node.type === "array" && (
        ...
        >
          ...
          <Button
            startIcon={<Add />}
            onClick={() =>
              onChange({ ...node, items: [...node.items, newNode("object")] })
            }
            sx={{ alignSelf: "flex-start" }}
          >
            {t("webhooks.item")}
    ...
    <Box>
      <Stack direction="row" spacing={1} alignItems="center" mb={1}>
        ...
        <Typography variant="subtitle2">
          {t("webhooks.payloadStructure")}
        </Typography>
      </Stack>
      <NodeEditor
        node={node}
        variables={variables}
        onChange={(next) => onChange(nodeToTemplate(next))}
      />
    </Box>
  );
};
