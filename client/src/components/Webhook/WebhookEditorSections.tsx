    ...
    <>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
        <Select
          value={provider}
          onChange={(e) =>
            onProviderChange(e.target.value as WebhookInput["provider"])
          }
          sx={{ minWidth: 180 }}
        >
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Stack
        ...
        mb={1}
      >
        <Typography variant="subtitle2">{t("webhooks.headers")}</Typography>
        <Button
          size="small"
          onClick={() =>
            onChange([...headers, { name: "", value: "", isSecret: false }])
          }
        >
        ...
        {headers.map((header, index) => (
          ...
          >
            ...
            <TextField
              ...
              label={
                header.isSecret
                  ? t("webhooks.secretValue")
                  : t("webhooks.headerValue")
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    update(index, { isSecret: e.target.checked })
                  }
                />
              }
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Tabs value={mode} onChange={(_, value) => onModeChange(value)}>
        <Tab value="builder" label={t("webhooks.builder")} />
        ...
        {mode === "builder" ? (
          ...
          <Stack spacing={1.5}>
            ...
            <TextField
              ...
              error={!!jsonError}
              helperText={jsonError ?? t("webhooks.jsonHint")}
          ...
          {rows.map((delivery) => (
            ...
            >
              <Typography variant="body2">
                {formatter.format(new Date(delivery.createdAt))}
              </Typography>
              <Typography variant="body2">{delivery.eventType}</Typography>
              <Typography
                variant="body2"
                color={delivery.success ? "success.main" : "error.main"}
              >
                {delivery.success
                  ? `${delivery.statusCode ?? "OK"}`
                  : (delivery.errorMessage ?? t("webhooks.failed"))}{" "}
