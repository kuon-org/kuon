...
export const TagArticles = ({ tag }: TagArticlesProps) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get("/articles", { params: { q, page, limit: 10 } });
      return res.data;
    },
  });
  ...
  return (
    <>
      ...
      {isLoading ? (
        <Stack>{[...Array(5)].map((_, i) => <ArticlesSkeleton key={i} />)}</Stack>
      ) : (
        <>
          {data?.articles.length > 0 ? (
            <>
              <Stack>{data.articles.map((article: any) => <ArticleCard key={article.id} article={article} />)}</Stack>
              <Pagination
                ...
                shape="rounded"
                sx={{ mx: "auto" }}
                renderItem={(item) => item.disabled ? <PaginationItem {...item} /> : (
                  <Link to={tagProfileRoute.to} search={{ page: item.page ?? 1 }} params={{ slug }}>
                    <PaginationItem {...item} />
                  </Link>
                )}
              />
            ...
            <Box sx={{ textAlign: "center", py: 10 }}>
              <Typography color="text.secondary">{t("articles.empty")}</Typography>
              <Typography variant="body2" color="text.secondary" mt={1}>{t("articles.emptyHint")}</Typography>
            </Box>
          )}
        </>
      )}
