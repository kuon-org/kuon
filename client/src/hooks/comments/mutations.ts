...
export const useCreateComment = (articleId: string) => {
  ...
  return useMutation({
    mutationFn: (payload: CreateCommentPayload) => createComment(articleId, payload),
    onSuccess: () => {
...
export const useToggleCommentLike = (articleId: string, commentId: string) => {
  ...
  return useMutation({
    ...
    onMutate: async () => {
      ...
      await queryClient.cancelQueries({ queryKey: commentKeys.isLiked(commentId) });
      await queryClient.cancelQueries({ queryKey: commentKeys.likeUsers(commentId) });

      const prevComment = queryClient.getQueryData<Comment>(commentKeys.detail(commentId));
      const prevIsLiked = queryClient.getQueryData<IsLikedResponse>(commentKeys.isLiked(commentId));
      ...
      if (prevComment && prevIsLiked) {
        ...
        queryClient.setQueryData<IsLikedResponse>(commentKeys.isLiked(commentId), {
    ...
    onError: (_error, _variables, context) => {
      if (context?.prevComment) {
        queryClient.setQueryData(commentKeys.detail(commentId), context.prevComment);
      }
      if (context?.prevIsLiked) {
        queryClient.setQueryData(commentKeys.isLiked(commentId), context.prevIsLiked);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: commentKeys.detail(commentId) });
      queryClient.invalidateQueries({ queryKey: commentKeys.likeUsers(commentId) });
      queryClient.invalidateQueries({ queryKey: commentKeys.isLiked(commentId) });
      queryClient.invalidateQueries({ queryKey: commentKeys.list(articleId) });
    },
  });
};
