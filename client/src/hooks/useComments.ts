...
export const useComments = (articleId?: string, commentId?: string) => {
    ...
    const createCommentMutation = useMutation({
        mutationFn: async (payload: { body: string; parent_comment_id?: string | null }) => {
        ...
        onError: (err: any) => {
            alert(err.response?.data?.message ?? "コメントの投稿に失敗しました");
        },
    });

    const softDeleteCommentMutation = useMutation({
        mutationFn: async (commentId: string) => {
            const res = await apiClient.delete(`/articles/${articleId}/comments/${commentId}`)
            return res.data;
        },
        onSuccess: () => {
            // コメント一覧を最新に更新
    ...
    const commentLikeUserQuery = useQuery<LikeUserResponse>({
        ...
        queryFn: async () => {
            ...
            const res = await apiClient.get(`/articles/${articleId}/comments/${commentId}/likes`);
    ...
    const commentIsLikedQuery = useQuery<IsLikedResponse>({
        ...
        queryFn: async () => {
            if (!commentId) throw new Error("コメントIDが指定されていません");
            const res = await apiClient.get(`/articles/${articleId}/comments/${commentId}/islike`);
    ...
    const likeMutation = useMutation({
        ...
        onMutate: async () => {
            ...
            await queryClient.cancelQueries({ queryKey: ["commentIsLiked", commentId] });
            await queryClient.cancelQueries({ queryKey: ["commentLikeUser", commentId] });
            const prevComment = queryClient.getQueryData<Comment>(["comment", commentId]);
            const prevIsLiked = queryClient.getQueryData<IsLikedResponse>(["commentIsLiked", commentId]);
            if (prevComment && prevIsLiked) {
                const newLikeCount = prevComment.like_count + (prevIsLiked.isLike ? -1 : 1);
        ...
        onError: (_err, _vars, context) =>{
            ...
            if(context?.prevIsLiked)
                queryClient.setQueryData(["commentIsLiked", commentId], context.prevIsLiked)
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["comment", commentId]});
            queryClient.invalidateQueries({ queryKey: ["commentLikeUser", commentId]});
            queryClient.invalidateQueries({ queryKey: ["commentIsLiked", commentId]});
        }
    })
