...
const Login: React.FC = () => {
    const { login, login_isPending, serverError, activeIdp, activeIdp_isLoading } = useAuthQuery();
    ...
    return (
        ...
        <Container maxWidth="md">
            <Typography variant='h4' sx={{ mt: 4 }}>Kuon にログイン</Typography>
            <Divider sx={{ mt: 4 }} />
            <Box
                sx={{
                    mt: 4,
            ...
            >
                ...
                <Box sx={{ flex: 1 }}>
                    ...
                    >
                        ...
                        <form.Field name="identifier" validators={{ /* ... */ }}>
                            {(field) => (
                                <TextField
                                    fullWidth
                        ...
                        <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
                            {([canSubmit, isSubmitting]) => (
                                ...
                                >
                                    {login_isPending || isSubmitting ? <CircularProgress size={24} /> : 'ログイン'}
                                </Button>
                            )}
                        </form.Subscribe>
                    </form>
                    <Box sx={{ display: "flex", mx: "auto", justifyContent: "center" }}>
                        <NavButton path="/register" message="アカウント作成はこちら" variant="outlined" />
                    </Box>
                </Box>
            </Box>
        </Container>
