...
import { type NotificationPreferences, useNotificationPreferences } from "../../hooks/useNotifications";
...
export const Notifications = () => {
  ...
  const update = async (key: keyof NotificationPreferences, checked: boolean) => { if (preferences) await updatePreferences({ ...preferences, [key]: checked }); };
  ...
  return (
    <Paper elevation={0} sx={{ width: "100%", maxWidth: 760, p: 3, border: "1px solid", borderColor: "divider", borderRadius: 2 }}>
      ...
      <Divider sx={{ mb: 3 }} />
      {isLoading || !preferences ? <Box sx={{ display: "flex", justifyContent: "center", py: 4 }}><CircularProgress size={24} /></Box> : (
        <>
          ...
          {settings.map((setting, index) => (
            <Box key={setting.key}>
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, py: 1.5 }}>
                ...
                <FormControlLabel label="" sx={{ m: 0 }} control={<Switch checked={preferences[setting.key]} disabled={isUpdating} onChange={(event) => void update(setting.key, event.target.checked)} />} />
              </Box>
