...
export const APIKeySettings = () => {
  ...
  const apiKeysQuery = useApiKeysQuery(!!authUserQuery.data);
  ...
  const handleCreate = async () => {
    ...
    const result = await createApiKey.mutateAsync({ name: newKeyName, expiresAt });
    if (result?.rawKey) {
  ...
  return (
    <Stack spacing={3}>
      ...
      <TableContainer><Table size="small"><TableHead><TableRow><TableCell>{t("apiKeys.columns.name")}</TableCell><TableCell>{t("apiKeys.columns.prefix")}</TableCell><TableCell>{t("apiKeys.columns.expiresAt")}</TableCell><TableCell>{t("apiKeys.columns.lastUsedAt")}</TableCell><TableCell align="right" /></TableRow></TableHead><TableBody>
        {apiKeysQuery.data?.length === 0 && !apiKeysQuery.isLoading && <TableRow><TableCell colSpan={5} align="center" sx={{ py: 3, color: "text.secondary" }}>{t("apiKeys.empty")}</TableCell></TableRow>}
        {apiKeysQuery.data?.map((key) => <TableRow key={key.id}><TableCell sx={{ fontWeight: "medium" }}>{key.name}</TableCell><TableCell><code>{key.prefix}***</code></TableCell><TableCell>{key.expires_at ? dateFormatter.format(new Date(key.expires_at)) : t("apiKeys.neverExpires")}</TableCell><TableCell sx={{ color: "text.secondary", fontSize: "0.85rem" }}>{key.last_used_at ? dayjs(key.last_used_at).locale(locale).fromNow() : t("apiKeys.neverUsed")}</TableCell><TableCell align="right"><Tooltip title={t("apiKeys.revoke")}><IconButton size="small" color="error" disabled={revokeApiKey.isPending} onClick={() => { if (window.confirm(t("apiKeys.revokeConfirm"))) revokeApiKey.mutate(key.id); }}><DeleteIcon fontSize="small" /></IconButton></Tooltip></TableCell></TableRow>)}
      </TableBody></Table></TableContainer>
      <Dialog open={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} fullWidth maxWidth="xs"><DialogTitle>{t("apiKeys.dialog.title")}</DialogTitle><DialogContent><Stack spacing={3} sx={{ mt: 1 }}><TextField label={t("apiKeys.dialog.name")} placeholder={t("apiKeys.dialog.namePlaceholder")} fullWidth value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} /><TextField select label={t("apiKeys.dialog.expiresAt")} value={expiryDays} onChange={(e) => setExpiryDays(e.target.value)} fullWidth><MenuItem value="7">{t("apiKeys.dialog.days", { count: 7 })}</MenuItem><MenuItem value="30">{t("apiKeys.dialog.days", { count: 30 })}</MenuItem><MenuItem value="90">{t("apiKeys.dialog.days", { count: 90 })}</MenuItem><MenuItem value="never">{t("apiKeys.neverExpires")}</MenuItem></TextField></Stack></DialogContent><DialogActions sx={{ px: 3, pb: 2 }}><Button onClick={() => setIsCreateModalOpen(false)} color="inherit">{t("common.cancel")}</Button><Button variant="contained" onClick={handleCreate} disabled={!newKeyName || createApiKey.isPending}>{t("apiKeys.dialog.issue")}</Button></DialogActions></Dialog>
