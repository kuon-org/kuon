        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell>{t("apiKeys.columns.expiresAt")}</TableCell>
              <TableCell>{t("apiKeys.columns.lastUsedAt")}</TableCell>
              <TableCell align="right" />
            </TableRow>
          </TableHead>
          <TableBody>
            {apiKeysQuery.data?.length === 0 && !apiKeysQuery.isLoading && (
              <TableRow>
                <TableCell
                  colSpan={5}
                  align="center"
            ...
            {apiKeysQuery.data?.map((key) => (
              <TableRow key={key.id}>
                ...
                <TableCell>
                  {key.expires_at
                    ? dateFormatter.format(new Date(key.expires_at))
                ...
                >
                  {key.last_used_at
                    ? dayjs(key.last_used_at).locale(locale).fromNow()
                    : t("apiKeys.neverUsed")}
                </TableCell>
                <TableCell align="right">
      ...
      >
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <TextField
              select
              label={t("apiKeys.dialog.expiresAt")}
              value={expiryDays}
              onChange={(e) => setExpiryDays(e.target.value)}
              fullWidth
            >
              <MenuItem value="7">
                {t("apiKeys.dialog.days", { count: 7 })}
              </MenuItem>
              <MenuItem value="30">
                {t("apiKeys.dialog.days", { count: 30 })}
              </MenuItem>
              <MenuItem value="90">
                {t("apiKeys.dialog.days", { count: 90 })}
              </MenuItem>
              <MenuItem value="never">{t("apiKeys.neverExpires")}</MenuItem>
            </TextField>
          </Stack>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          >
            {t("apiKeys.dialog.issue")}
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog open={showKeyModalOpen} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <KeyIcon color="success" /> {t("apiKeys.generated.title")}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2}>
            <Typography
