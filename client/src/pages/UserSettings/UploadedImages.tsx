...
export const UploadedImages = () => {
  ...
  if (uploadedImagesQuery.isLoading)
    return shell(<Typography>{t("uploads.loading")}</Typography>);
  if (!uploadedImages || uploadedImages.length === 0)
    return shell(<Typography>{t("uploads.empty")}</Typography>);
            ...
            <Box sx={{ display: "flex", alignItems: "center", my: 2, gap: 2 }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", flex: 1 }}>
                ...
                <Typography sx={{ flex: 1 }}>
                  {t("uploads.uploadedAt", {
                    date: img.created_at ? formatDate(img.created_at) : "-",
                  })}
                </Typography>
                <Typography sx={{ flex: 1 }}>
                  {t("uploads.size", { size: img.size_bytes ?? 0 })}
                </Typography>
              </Box>
