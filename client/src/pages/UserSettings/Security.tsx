...
export const Security = () => {
  ...
  const sorted = [...(sessionDevicesQuery.data ?? [])].sort(
    (a, b) =>
            ...
            <Stack direction="row" spacing={3} alignItems="center">
              ...
              >
                <Box>
                  ...
                  >
                    <Typography fontWeight="bold">
                      {sd.device_name ?? t("security.unknownDevice")}
                    </Typography>
                    {sd.is_current && (
                      <Chip
                        label={t("security.currentSession")}
                        color="primary"
                        size="small"
                      />
                    )}
                  ...
                  {sd.last_used_at && (
                    <Typography variant="body2" color="text.secondary">
                      {t("security.lastUsedAt", {
                        date: formatDate(sd.last_used_at),
                      })}
                    </Typography>
                  )}
                </Box>
                <Tooltip
                  title={
                    sd.is_current
                      ? t("security.currentSessionCannotRevoke")
                      : ""
                  }
                  placement="top"
                  arrow
                >
                  <span>
                    <Button
                      ...
                      color="error"
                      size="small"
                      disabled={logoutSession.isPending || sd.is_current}
                      onClick={() => logoutSession.mutate(sd.id)}
