...
export const Account = () => {
  ...
  const { user, updateAccount, updateAccount_isPending, successMessage, serverError, activeIdp, activeIdp_isLoading, userAvatars, switchAvatar, switchAvatar_isPending, userIdentities, unlink, unlink_isPending } = useAuthQuery();
  const form = useForm({ defaultValues: { username: user?.username ?? "" }, onSubmit: async ({ value }) => { await updateAccount(value); } });
    ...
    <Paper sx={{ mx: "auto", flex: 1, p: 3, minWidth: { xs: "100%", sm: "100%", md: "600px", lg: "850px" } }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Grid container spacing={2}>
          {userAvatars?.map((avatar) => (
            <Grid key={avatar.id}>
              <Paper variant="outlined" sx={{ p: 1, display: "flex", flexDirection: "column", alignItems: "center", borderColor: avatar.is_selected ? "primary.main" : "divider", bgcolor: avatar.is_selected ? "action.selected" : "background.paper", cursor: "pointer", minWidth: 100 }} onClick={() => !avatar.is_selected && switchAvatar(avatar.id)}>
                ...
                <Radio checked={avatar.is_selected} disabled={switchAvatar_isPending} size="small" />
      ...
      <form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit(); }}>
        ...
        <Stack spacing={2}>
          ...
          <Button variant="contained" size="small" type="submit" disabled={updateAccount_isPending} sx={{ alignSelf: "flex-start", mt: 1, width: "fit-content" }}>{updateAccount_isPending ? t("common.updating") : t("common.update")}</Button>
        </Stack>
      </form>
      <Divider sx={{ mt: 3 }} />
