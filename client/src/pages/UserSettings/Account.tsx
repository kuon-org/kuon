...
export const Account = () => {
    ...
    return (
        ...
        >
            ...
            <Box sx={{ mb: 4 }}>
                <Typography variant="subtitle2" sx={{ mb: 1 }}>使用するアイコン</Typography>

                <Grid container spacing={2}>
                    {/* 既存のアバター一覧を表示 */}
                    {userAvatars?.map((avatar) => (
                        <Grid key={avatar.id}> {/* itemキーワードを追加してGridを正しく構成 */}
                            <Paper
                                variant="outlined"
                                sx={{
                                    p: 1,
                                    ...
                                    bgcolor: avatar.is_selected ? 'action.selected' : 'background.paper',
                                    cursor: 'pointer',
                                    minWidth: 100
                                }}
                    ...
                    {!hasLocalAvatar && (
                        <Grid>
                            ...
                            >
                                <Link
                                    to={accountCustomImageRoute.to}
                                    style={{ textAlign: 'center', fontSize: '0.875rem', textDecoration: 'none' }}
                                >
                                    ＋ 画像を<br />アップロード
                                </Link>
                            </Paper>
                    ...
                    return (
                        <Button
                            ...
                            onClick={(e) => {
                                if (linked) {
                                    e.preventDefault();
                                    if (window.confirm(`${idp.display_name} との連携を解除しますか？`)) {
