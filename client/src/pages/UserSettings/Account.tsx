...
export const Account = () => {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      await updateUsername.mutation.mutateAsync(value);
    },
  });
          ...
          {userAvatars?.map((avatar) => (
            <Grid key={avatar.id}>
              <Paper
                ...
                onClick={() =>
                  !avatar.is_selected && switchAvatar.mutate(avatar.id)
                }
              >
                <Avatar
                ...
                <Radio
                  ...
                  disabled={switchAvatar.isPending}
                  size="small"
                />
        ...
        <Stack spacing={2}>
          <form.Field
            ...
            validators={{
              onChange: ({ value }) =>
                !value
                  ? t("common.required", { field: t("account.usernameField") })
                  : value.length > 50
                    ? t("common.maxLength", { max: 50 })
                    : undefined,
            }}
            children={(field) => (
              <TextField
          ...
          <Button
            ...
            size="small"
            type="submit"
            disabled={updateUsername.mutation.isPending}
            sx={{ alignSelf: "flex-start", mt: 1, width: "fit-content" }}
          >
            {updateUsername.mutation.isPending
              ? t("common.updating")
              : t("common.update")}
          </Button>
        </Stack>
      ...
      <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 1 }}>
        ...
        {activeIdpQuery.data?.map((idp) => {
          ...
          return (
                ...
                idp.logo_url ? (
                  <img
                    ...
                    style={{
                      ...
                      height: 20,
                      borderRadius: 4,
                      filter: linked ? "brightness(0) invert(1)" : "none",
                    }}
                  />
                ) : null
              }
                  ...
                  if (
                    window.confirm(
                      t("account.unlinkConfirm", {
                        provider: idp.display_name,
                      }),
                    )
                  )
                    unlinkIdentity.mutate(idp.provider_name);
                }
              }}
            ...
            >
              {linked
                ? t("account.unlink", { provider: idp.display_name })
                : t("account.link", { provider: idp.display_name })}
            </Button>
          );
        })}
      </Box>
