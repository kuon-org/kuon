...
import { useCreatePickupArticle, useDeletePickupArticle, usePickupArticlesQuery } from "../../hooks/users";
...
export const PickupArticles = ({ userId, isMe }: { userId: string; isMe: boolean; }) => {
  ...
  const handleTogglePickup = async (articleId: string, isPicked: boolean) => {
    try { if (isPicked) await deletePickup.mutateAsync(articleId); else await createPickup.mutateAsync(articleId); }
    catch (err) { console.error("Pickup toggle failed", err); }
  };

  if (pickupQuery.isLoading) return <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>{[1, 2, 3].map((i) => <Paper key={i} sx={{ mx: "auto", width: { xs: "100%", sm: "600px" }, height: "100px" }}><LoadingSkelton /></Paper>)}</Box>;
  ...
  return <>{renderContent()}<PickupArticleDialog open={isDialogOpen} onClose={() => setIsDialogOpen(false)} userId={userId} currentPickups={pickup} onToggle={handleTogglePickup} isSubmitting={createPickup.isPending || deletePickup.isPending} /></>;
};
