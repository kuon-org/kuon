...
export const StockPage = () => {
  ...
  if (!detail)
    return <Typography sx={{ p: 4 }}>{t("stock.notFound")}</Typography>;
  const isMe = authUser?.id === detail.users.id;
  const formattedDate = new Date(detail.created_at).toLocaleString(
    i18n.language,
  );
        ...
        <Box sx={{ width: { xs: "100%", md: "300px" }, flexShrink: 0 }}>
          ...
          >
            <Stack spacing={2.5}>
              ...
              >
                {detail.description || t("stock.noDescription")}
              </Typography>
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                {detail.stock_list_tags?.map((item: any) => (
                  <TagChip key={item.tags.id} tag={item.tags} />
                ))}
              </Box>
              <Stack spacing={1}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <Typography variant="caption" fontWeight="bold">
                    {t("stock.articleCount", { count: detail.totalCount || 0 })}
                  </Typography>
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <AccessTimeIcon fontSize="small" color="action" />
                  <Typography variant="caption">
                    {t("stock.createdAt", { date: formattedDate })}
                  </Typography>
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <LikeButton
                    isLiked={stockLike.data ?? false}
                    isLikePending={likeStock.isPending}
                    mutateLike={() => likeStock.mutate()}
                  />
                  <Typography variant="caption">
                    {detail._count?.stock_list_likes || 0}
                  </Typography>
                </Box>
              </Stack>
            </Stack>
          ...
          >
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
              <Link
                to="/$username"
                params={{ username: detail.users.username }}
              >
                <Avatar
            ...
            <Typography
              ...
              sx={{ whiteSpace: "pre-wrap" }}
            >
              {detail.users.bio || t("stock.noBio")}
            </Typography>
            {!isMe && isAuth && (
              <Button
                variant={
                  isFollowingQuery.data?.isFollow ? "outlined" : "contained"
                }
                disabled={follow.isPending}
                sx={{ mt: 2, width: "fit-content" }}
                onClick={() => follow.mutate(detail.users.id)}
              >
