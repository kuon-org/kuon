    ...
    <Box>
      ...
      {q && (
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          {t("stock.searchResult", { query: q })}
        </Typography>
      )}
      <Stack>
        {detail.stock_items.map((item) => (
          <ArticleCard key={item.id} article={item.articles} />
        ))}
      </Stack>
      {detail.pagination && detail.pagination.totalPages > 1 && (
        <Box sx={{ display: "flex", justifyContent: "center", mt: 6, mb: 4 }}>
          <Pagination
            count={detail.pagination.totalPages}
            page={page}
            variant="outlined"
            shape="rounded"
            renderItem={(item) =>
