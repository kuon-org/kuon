...
export const StockEditPages = ({ initialData }: StockEditPageProps) => {
  ...
  const { createList, updateList, isCreating, isUpdating, deleteList, isDeleting } = useStocks();
  ...
  const [selectedTags, setSelectedTags] = useState<string[]>(initialData?.stock_list_tags?.map((item: any) => item.tags.name) || []);
  const [isDefault, setIsDefault] = useState(initialData?.is_default || false);

  const handleDelete = async () => {
    if (!initialData?.id || !window.confirm(t("stock.edit.deleteConfirm"))) return;
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = { name, visibility: visibility as any, description, tagIds, is_default: isDefault };
      if (isEdit) {
        await updateList({ listId: initialData.id, payload });
        navigate({ to: stocksDetailsRoute.to, search: { page: 1, q: undefined }, params: { listId: initialData.id } });
      } else {
  ...
  return (
    <Paper sx={{ p: 4, borderRadius: 2 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Typography variant="h6" fontWeight="bold" mb={3}>{isEdit ? t("stock.edit.title") : t("stock.edit.newTitle")}</Typography>
        <Typography variant="h6" color="text.secondary" mb={3}>{isEdit && !isSystem && <Button color="error" onClick={handleDelete} disabled={isDeleting}>{t("stock.edit.delete")}</Button>}</Typography>
      </Box>
