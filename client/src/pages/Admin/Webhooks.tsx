...
export const Webhooks = () => {
  ...
  const webhooksEnabled =
    settings?.find((s) => s.key === "webhooks_enabled")?.value === "true";
  const allowUserWebhooks =
    settings?.find((s) => s.key === "allow_user_webhooks")?.value === "true";
  ...
  if (metadataQuery.isLoading)
    return <Typography>{t("webhooks.loading")}</Typography>;
  ...
  return (
    <Box sx={{ width: "100%", maxWidth: 1040, pb: 8 }}>
      ...
      >
        ...
        <Tooltip title={t("webhooks.helpTooltip")}>
      ...
      <Stack spacing={2}>
        <Paper variant="outlined" sx={{ p: 2 }}>
          ...
          <Stack>
            ...
            <FormControlLabel
              control={
                <Switch
                  checked={allowUserWebhooks}
                  disabled={!webhooksEnabled || updateServerSetting.isPending}
                  onChange={(_, checked) =>
                    updateServerSetting.mutate({
                      key: "allow_user_webhooks",
                      value: String(checked),
                    })
                  }
                />
              }
              label={t("webhooks.allowUser")}
            />
          </Stack>
          {!webhooksEnabled && (
            <Alert severity="info" sx={{ mt: 1 }}>
              {t("webhooks.disabledInfo")}
            </Alert>
          )}
        </Paper>
        <Paper variant="outlined" sx={{ p: 2 }}>
          ...
          >
            ...
            <Button startIcon={<Add />} onClick={resetEditor}>
              {t("webhooks.new")}
            </Button>
          </Stack>
                  ...
                  >
                    <Box sx={{ minWidth: 0 }}>
                      ...
                      >
                        ...
                        <Chip
                          size="small"
                          label={providerLabel[webhook.provider]}
                        />
                        <Chip
                          size="small"
                          variant="outlined"
                    ...
                    <Stack direction="row" alignItems="center">
                      ...
                      <Tooltip title={t("webhooks.edit")}>
                        <IconButton onClick={() => loadWebhook(webhook.id)}>
                          <Edit />
                        </IconButton>
                      </Tooltip>
                        ...
                        <IconButton
                          ...
                          onClick={async () => {
                            if (
                              ...
                              if (editingId === webhook.id) resetEditor();
                            }
                          }}
                        >
                          <Delete />
        ...
        >
          ...
          {editingId && (
            <Button startIcon={<Refresh />} onClick={resetEditor}>
              {t("webhooks.new")}
            </Button>
          )}
        </Stack>
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
          <Button
            variant="outlined"
            disabled={!!jsonError || previewPayload.isPending}
            onClick={async () =>
              setPreview(
                (
                  await previewPayload.mutateAsync({
                    payloadTemplate: payload,
                    eventType: event,
                  })
                ).payload,
          ...
          <Button
            variant="outlined"
            disabled={!url || !!jsonError || testSend.isPending}
            onClick={async () =>
              setTestResult(
          ...
          <Button
            variant="contained"
            disabled={
              !url || !name || !event || !!jsonError || saveWebhook.isPending
            }
            onClick={async () => {
              await saveWebhook.mutateAsync({ id: editingId, input: input() });
              resetEditor();
            }}
          >
            {editingId ? t("webhooks.update") : t("webhooks.save")}
          </Button>
        </Stack>
        {preview !== undefined && (
          <Paper variant="outlined" sx={{ p: 2 }}>
        ...
        {testResult && (
          <Alert severity={testResult.ok ? "success" : "warning"}>
            {t("webhooks.testSend")}: HTTP {testResult.status ?? "-"} /{" "}
        ...
        {editingId && (
          <WebhookDeliveryLog
            ...
            title={t("webhooks.recentDeliveries")}
            limit={10}
          />
        )}
      </Stack>
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>{t("webhooks.help.title")}</DialogTitle>
        <DialogContent>
          <Stack spacing={2} pt={1}>
            {[
              ...
              ["valuesTitle", "valuesDescription"],
              ["testTitle", "testDescription"],
            ].map(([title, description]) => (
              <Box key={title}>
                <Typography fontWeight={600}>
                  {t(`webhooks.help.${title}`)}
                </Typography>
                <Typography color="text.secondary">
                  {t(`webhooks.help.${description}`)}
                </Typography>
              </Box>
            ))}
          </Stack>
