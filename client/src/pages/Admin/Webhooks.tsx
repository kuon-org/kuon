...
export const Webhooks = () => {
  ...
  const resetEditor = () => { setEditingId(undefined); setName("Article published"); setProvider("generic"); setUrl(""); setEvent(metadata?.events[0]?.type ?? "article.published"); setHeaders([]); syncPayload(defaultPayload); setPreview(undefined); setTestResult(undefined); setDeliveries(undefined); setMode("builder"); };
  ...
  return <Box sx={{ width: "100%", maxWidth: 1040, pb: 8 }}>
    ...
    <Stack spacing={2}>
      ...
      <TextField label={t("webhooks.name")} value={name} onChange={(e) => setName(e.target.value)} /><TextField label="Webhook URL" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://..." />
