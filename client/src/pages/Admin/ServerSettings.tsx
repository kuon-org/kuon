...
export const ServerSettings = () => {
  ...
  const updateSetting = async (key: string, value: string) => {
    setError(null);
    try { await updateServerSetting({ key, value }); }
  ...
  return (
    <Paper elevation={0} sx={{ mx: "auto", p: 3, minWidth: { xs: "100%", md: "600px", lg: "850px" }, border: "1px solid", borderColor: "divider", borderRadius: 2 }}>
      ...
      {settings_isLoading ? <Box sx={{ display: "flex", justifyContent: "center", py: 2 }}><CircularProgress size={24} /></Box> : (
        <>
          ...
          <FormControlLabel control={<Switch checked={maintenanceMode} onChange={(event) => updateBooleanSetting("maintenance_mode", event.target.checked)} disabled={maintenanceSetting?.readOnly || updateServerSetting_isPending} />} label={maintenanceMode ? t("common.enabled") : t("common.disabled")} />
          <Divider sx={{ my: 3 }} />
          ...
          <FormControlLabel control={<Switch checked={notificationsEnabled} onChange={(event) => updateBooleanSetting("notifications_enabled", event.target.checked)} disabled={notificationsSetting?.readOnly || updateServerSetting_isPending} />} label={notificationsEnabled ? t("common.enabled") : t("common.disabled")} />
          <Divider sx={{ my: 3 }} />
          ...
          <FormControlLabel control={<Switch checked={webhooksEnabled} onChange={(event) => updateBooleanSetting("webhooks_enabled", event.target.checked)} disabled={webhooksSetting?.readOnly || updateServerSetting_isPending} />} label={webhooksEnabled ? t("common.allowed") : t("common.notAllowed")} />
          <Box sx={{ mt: 2, ml: 2 }}>
            ...
            <FormControlLabel control={<Switch checked={allowUserWebhooks} onChange={(event) => updateBooleanSetting("allow_user_webhooks", event.target.checked)} disabled={!webhooksEnabled || allowUserWebhooksSetting?.readOnly || updateServerSetting_isPending} />} label={allowUserWebhooks ? t("common.allowed") : t("common.notAllowed")} />
          </Box>
          <Divider sx={{ my: 3 }} />
          <SmtpSettingsSection />
        </>
