...
export const Dashboard = () => {
  ...
  const requireAuthentication = getBooleanSetting(
    settings,
    "require_authentication",
  );
  const notificationsEnabled = getBooleanSetting(
    settings,
        ...
        <>
          {settingsQuery.isLoading ? (
            ...
            >
              <StatusCard
                ...
                description={t("dashboard.maintenance.description")}
                enabled={maintenanceMode}
                enabledLabel={t("dashboard.maintenance.enabled")}
                disabledLabel={t("dashboard.maintenance.disabled")}
                warningWhenEnabled
              />
              <StatusCard
                title={t("dashboard.authentication.title")}
                description={t("dashboard.authentication.description")}
                enabled={requireAuthentication}
                enabledLabel={t("dashboard.authentication.enabled")}
                disabledLabel={t("dashboard.authentication.disabled")}
              />
              <StatusCard
                title={t("dashboard.localRegistration.title")}
                description={t("dashboard.localRegistration.description")}
                enabled={allowLocalAccountRegistration}
                enabledLabel={t("dashboard.localRegistration.enabled")}
                disabledLabel={t("dashboard.localRegistration.disabled")}
              />
              <StatusCard
              ...
              />
              <StatusCard
                title={t("dashboard.webhooks.title")}
                description={t("dashboard.webhooks.description")}
          ...
          >
            ...
            {statusQuery.isLoading && (
              <Typography color="text.secondary">
                {t("dashboard.runtime.loading")}
              </Typography>
            )}
            {statusQuery.isError && (
              <Typography color="error">
                {t("dashboard.runtime.loadFailed")}
              </Typography>
            )}
            {status && (
              <>
                ...
                >
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      {t("dashboard.runtime.database")}
                    </Typography>
                    <Typography>
                      {status.database.status === "connected"
                        ? t("dashboard.runtime.connected")
                        : t("dashboard.runtime.error")}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                    ...
                    <Typography sx={{ wordBreak: "break-word" }}>
                      {status.database.postgresVersion ??
                        t("dashboard.runtime.unknown")}
                    </Typography>
                  </Box>
                </Box>
                <Divider sx={{ my: 2.5 }} />
                <Typography variant="subtitle1" sx={{ mb: 1.5 }}>
                  {t("dashboard.runtime.environmentVariables")}
                </Typography>
                    ...
                    >
                      <Box>
                        ...
                        <Typography
                          ...
                          sx={{ ml: 1 }}
                        >
                          {t("dashboard.runtime.sourceLabel")}
                        </Typography>
                      </Box>
                      <Chip
                        size="small"
