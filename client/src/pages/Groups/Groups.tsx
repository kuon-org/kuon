...
export const Groups = () => {
  ...
  const feed = useQuery({
    queryKey: ["groups", "feed", page],
    queryFn: () => fetchGroupFeed(page),
  });
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      setForm({ name: "", display_name: "", slug: "", description: "" });
    },
  });
    ...
    <Container sx={{ py: 4 }}>
          ...
          >
            <CardContent>
              ...
              <Typography variant="caption">
                {t("list.counts", {
                  members: group._count.user_groups,
                  articles: group._count.articles,
                })}
              </Typography>
            </CardContent>
          </Card>
      ...
      {feed.data && feed.data.totalPages > 1 && (
        <Box display="flex" justifyContent="center" mt={4}>
          <Pagination
            count={feed.data.totalPages}
            page={page}
            onChange={(_event, nextPage) =>
              navigate({ to: "/groups", search: { page: nextPage } })
            }
          />
        </Box>
      )}
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            disabled={
              create.isPending || !form.name || !form.display_name || !form.slug
            }
