...
import { useDeleteArticle, useRollbackArticle, useUserArticlesQuery } from "../../hooks/articles";
...
export const Drafts = () => {
  ...
  const userArticlesQuery = useUserArticlesQuery();
  const rollbackArticle = useRollbackArticle();
  const deleteArticle = useDeleteArticle();
  const navigate = useNavigate();
  const theme = useTheme();
  const isSmall = useMediaQuery(theme.breakpoints.down("sm"));
  const articles = userArticlesQuery.data ?? [];
