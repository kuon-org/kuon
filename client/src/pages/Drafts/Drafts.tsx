...
export const Drafts = () => {
  ...
  const filteredDrafts = showUnpublishedOnly
    ...
    : articles;
  const selectedArticle = filteredDrafts.find(
    (article) => article.id === selectedArticleId,
  );
  const handleRollback = (id: string, title: string) => {
    if (window.confirm(t("drafts.rollbackConfirm", { title })))
      rollbackArticle.mutate(id);
  };
  const handleDelete = (id: string, title: string) => {
    if (window.confirm(t("drafts.deleteConfirm", { title })))
    ...
    >
      ...
      >
        ...
        <FormControlLabel
          ...
          label={t("drafts.unpublishedOnly")}
        />
        <List sx={{ mt: 2, overflowY: "scroll" }}>
          {userArticlesQuery.isLoading ? (
            <ListItem>
              <ListItemText primary={t("status.loading")} />
            </ListItem>
          ) : (
            filteredDrafts.map((article) => {
              const hasDraftDiff =
                article.is_published && article.status === "draft";
              return (
                <ListItemButton
                  key={article.id}
                  divider
                ...
                >
                  <Box sx={{ display: "flex", gap: 1, width: "100%", mb: 0.5 }}>
                    <Typography variant="caption" color="text.secondary">
                      {article.is_published && t("drafts.published")}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      {t("drafts.updatedAt", {
                        date: new Date(article.updated_at).toLocaleString(
                          i18n.language,
                        ),
                      })}
                    </Typography>
                  ...
                  <ListItemText
                    ...
                    secondary={
                      article.raw_content.trim()
                        ? article.raw_content.substring(0, 50) +
                          (article.raw_content.length > 50 ? "..." : "")
                        : t("drafts.emptyBody")
                    }
                  />
                  <Box sx={{ display: "flex", gap: 1, width: "100%", mb: 0.5 }}>
                    ...
                    {hasDraftDiff && (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleRollback(article.id, article.title);
                        }}
                      >
                        {t("drafts.rollback")}
                      </Button>
          ...
          {!userArticlesQuery.isLoading && filteredDrafts.length === 0 && (
            <ListItem>
              <ListItemText primary={t("drafts.empty")} />
            </ListItem>
          )}
        </List>
      </Paper>
