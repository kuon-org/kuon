...
export const Drafts = () => {
  ...
  const handleDelete = (id: string, title: string) => {
    if (
      window.confirm(`「${title}」を完全に削除しますか？ゴミ箱へ移動します。`)
    ) {
      deleteArticle(id);
    }
  };
              ...
              return (
                ...
                >
                  <Box sx={{ display: "flex", gap: 1, width: "100%", mb: 0.5 }}>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      最終更新: {new Date(a.updated_at).toLocaleString()}
                    </Typography>
                  </Box>
                  <ListItemText
                    ...
                    secondary={
                      a.raw_content.trim()
                        ? a.raw_content.substring(0, 50) +
                          (a.raw_content.length > 50 ? "..." : "")
                        : "本文未入力"
                    }
                  />
                  <Box sx={{ display: "flex", gap: 1, width: "100%", mb: 0.5 }}>
