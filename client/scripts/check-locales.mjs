...
for (const locale of localeNames) {
  ...
  for (const file of files) {
    ...
    resources[locale][namespace] =
      parse(await readFile(path.join(localesRoot, locale, file), "utf8")) ?? {};
  }
}
...
for (const locale of localeNames) {
  ...
  const extraNamespaces = namespaces.filter(
    (namespace) => !sourceNamespaces.includes(namespace),
  );
  if (missingNamespaces.length)
    problems.push(`missing namespaces: ${missingNamespaces.join(", ")}`);
  if (extraNamespaces.length)
    problems.push(`extra namespaces: ${extraNamespaces.join(", ")}`);
  for (const namespace of sourceNamespaces.filter((name) =>
    ...
    if (extraKeys.length)
      problems.push(`${namespace}: extra keys: ${extraKeys.join(", ")}`);
  }
  if (problems.length)
    throw new Error(
      `Locale '${locale}' does not match '${sourceLocale}':\n- ${problems.join("\n- ")}`,
    );
}
