...
export class ArticlesController {
  ...
  getArticles = async (req: Request, res: Response) => {
    try {
      const articles = await this.articlesService.getPublishedArticleList();
      res.json(articles);
    } catch (error: any) {
  ...
  getArticleMarkdown = async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.status(error.message === "ArticleNotFound" ? 404 : 500).json({ message: error.message });
    }
  };
  ...
  uploadArticleImage = async (req: AuthRequest, res: Response) => {
    ...
    upload(req, res, async (err: any) => {
      if (err || !req.file) return res.status(500).json({ message: "アップロード失敗" });
