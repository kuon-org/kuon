...
export class UsersRepository {
    ...
    async createUser(id: UUID, username: string, email: string, displayName?: string): Promise<users> {
    ...
    async findLocalAccountByUsername(username: string): Promise<local_accounts | null> {
        return prisma.local_accounts.findFirst({
            where: { users: { username } }
        });
    }

    async findLocalAccountByUserId(userId: string): Promise<local_accounts | null> {
    ...
    async createLocalAccount(
        ...
        password: string,
        displayName?: string
    ): Promise<{ user: users; account: local_accounts; security: any; role: any }> {
    ...
    async updatePassword(userId: string, newPassword: string): Promise<local_accounts> {
    ...
    async update2FASetting(userId: string, secret: string, isEnabled?: boolean): Promise<user_security> {
    ...
    async unFollowUser(followerId: string, followeeId: string) {
        return prisma.user_follows.delete({
            where: {
                follower_id_followee_id: { follower_id: followerId, followee_id: followeeId },
            },
        });
    }
            ...
            include: {
                user_identities: {
                    select: {
                        ...
                        linked_at: true,
                        identity_providers: {
                            select: { display_name: true, provider_name: true, logo_url: true }
                        }
                    }
                },
                user_avatars: {
    ...
    async isAdmin(userId: string) {
        ...
        return user.user_roles.some(
            (ur): ur is { roles: { name: string } } & typeof ur => !!ur.roles && ur.roles.name === "admin"
        );
    }
