...
COMMENT ON COLUMN knowledge.user_notifications.reference_type IS '通知の参照先種別(article/comment/user等)';
...
ALTER TABLE knowledge.user_settings
    ...
    ADD COLUMN IF NOT EXISTS notify_on_user_follow BOOLEAN DEFAULT TRUE;
...
COMMENT ON COLUMN knowledge.user_settings.notify_on_user_follow IS '他ユーザからフォローされたときにアプリ内通知するか';
