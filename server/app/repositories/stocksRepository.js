...
export class StocksRepository {
    ...
    async createList(data) {
        return this.db.stock_lists.create({
            data: {
                ...
                stock_list_tags: data.tagIds && data.tagIds.length > 0
                    ? {
     ...
     */
    async findAllItemsByUserId(userId, page = 1, limit = 20, q) {
        ...
        const uniqueArticles = Array.from(new Map(items.map((item) => [item.article_id, item.articles])).values());
     ...
     */
    async findListsByUserId(userId, articleId) {
        ...
        return lists.map((list) => ({
            ...list,
            isStored: articleId ? list.stock_items.length > 0 : false,
     ...
     * リストに記事を追加（重複時はスキップ）
     */
    async addItem(stockListId, articleId) {
        // 現在のリスト内での最大順序を取得
     ...
     * リストから記事を削除
     */
    async removeItem(stockListId, articleId) {
     ...
     */
    async findPublicLists(page, limit) {
        const skip = (page - 1) * limit;
     ...
     */
    async findListDetail(listId, page = 1, limit = 20, q) {
        const skip = (page - 1) * limit;
        // 1. 検索条件の組み立て (ここがポイント！)
