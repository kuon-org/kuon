export class IdpConfigurationsService {
    ...
    async getAllProvidersList(userId) {
    ...
    async upsertIdp(userId, provider_name, data) {
        const isAdmin = await this.urepo.isAdmin(userId);
        if (!isAdmin)
            throw new Error("権限がありません");
    ...
    async toggleActive(userId, provider_name) {
        const isAdmin = await this.urepo.isAdmin(userId);
        if (!isAdmin)
            throw new Error("権限がありません");
        return await this.repo.toggleIdpActive(provider_name);
    }
    async deleteIdp(userId, provider_name) {
        const isAdmin = await this.urepo.isAdmin(userId);
        if (!isAdmin)
            throw new Error("権限がありません");
